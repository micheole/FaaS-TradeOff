...
and 95% confidence intervals, and saves the results to a CSV file.

Usage:
    python process_logs.py --input_dir path/to/logs --output_file results.csv
...
def main():
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.log'):
                ...
                # Extract Trial_Count from the directory structure
                # Assuming the structure: .../monte-carlo/aws/logs/log1000000.log
                path_parts = root.split(os.sep)
                try:
                    provider = path_parts[-4]  # e.g., monte-carlo
                    # Extract trial count from the log file name, e.g., log1000000.log
                    trial_count_match = re.search(r'trials-(\d+)', file, re.IGNORECASE)
                    if trial_count_match:
                        trial_count = int(trial_count_match.group(1))
                    else:
                        trial_count = None
                except IndexError:
                    function = 'Unknown'
    ...
    results_df = results_df.sort_values(by=['Provider', 'Trial_Count'])
